...
function add_content($DataDecode) {
	...
	if ($Header == true) {
		$wsconfigtag->appendChild($wsheadertag);
	}
	...
	foreach ($DataDecode as $value) {
		if ($value->temparray->JsonType == "Input") {
			...
			if (strcmp($value->temparray->FirstModule, $value->temparray->{'ws-select-multipleoptionGroup'}) == 0) {
				$concatstring = "";
				if (!empty($value->temparray->{'ws-input-static'})) {
					$concatstring .= $value->temparray->{'ws-input-static'} . ",";
				}
				...
				$concatstringrel = "";
				if (!empty($value->temparray->{'ws-input-static'})) {
					$concatstringrel .= $value->temparray->{'ws-input-static'} . ",";
				}
	...
	foreach ($DataDecode as $value) {
		if ($value->temparray->JsonType == "Output") {
			...
			if (strcmp($value->temparray->FirstModule, $value->temparray->{'ws-output-select-multipleoptionGroup'}) == 0) {
				$concatstring = "";
				if (!empty($value->temparray->{'ws-output-static'})) {
					$concatstring .= $value->temparray->{'ws-output-static'} . ",";
				}
				foreach ($value->temparray->Anotherdata as $values) {
					$concatstring .= explode(":", $values->DataValues)[2] . ",";
				}
				...
				$concatstringrel = "";
				if (!empty($value->temparray->{'ws-output-static'})) {
					$concatstringrel .= $value->temparray->{'ws-output-static'} . ",";
				}
				foreach ($value->temparray->Anotherdata as $valuea) {
					$concatstringrel .= explode(":", $valuea->DataValues)[2] . ",";
				}
	...
	if ($Output == true) {
		$root->appendChild($Outputtag);
	}
	if ($ValueMap == true) {
		$root->appendChild($valuemaptag);
	}
	if ($Errorhandler == true) {
		$root->appendChild($errorhandlertag);
	}
