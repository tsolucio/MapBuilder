...
function save_history($datas, $queryid, $xmldata) {
	...
	if (!empty($seq)) {
		...
		$adb->pquery(
			...
			array($idquery2, $datas->FirstModuleJSONvalue, $datas->FirstModuleJSONtext, $datas->SecondModuleJSONvalue,
			$datas->SecondModuleJSONtext,
			$xmldata,
			$seq,
			1,
			$datas->FirstModuleJSONfield,
			$datas->SecondModuleJSONfield,
			$datas->Labels)
		);
		//return $idquery;
	} else {
...
function add_description($datas) {
	if (!empty($datas)) {
		...
		$SecmodulenameText = $xml->createTextNode(trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace("(many)", '', preg_replace(
			'/\s+/',
			'',
			explode(";", $datas->SecondModuleJSONvalue)[0]
		)))));
...
function addsqlTag($returni, $QueryGenerate = '') {
