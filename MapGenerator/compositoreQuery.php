	...
	for ($i = 0; $i < count($selTab1); $i++) {
		...
		if ($i == 0) {
			...
			if ($usergroup=='user') {
				...
				$q2join=' JOIN vtiger_users User_'.strtolower($selTab2[$i]).'_'.$index.' on User_'.strtolower($selTab2[$i]).'_'.$index.'.id=CRM_'
				.strtolower($selTab2[$i]).'_'.$index.'.smownerid ';
				$strf=str_replace(
					array('CRM_'.strtolower($selTab1[$i]).'_0.smownerid','CRM_'.strtolower($selTab2[$i]).'_'.$index.'.smownerid'),
					array('User_'.strtolower($selTab1[$i]).'_0.user_name','User_'.strtolower($selTab2[$i]).'_'.$index.'.user_name'),
					substr($stringaFields2, 0, -2)
				);
			} elseif ($usergroup=='group') {
				$qjoin=' join vtiger_groups Group_'.strtolower($selTab1[$i]).'_0 on Group_'.strtolower($selTab1[$i]).'_0.groupid=CRM_'
				.strtolower($selTab1[$i]).'_0.smownerid ';
				$q2join=' join vtiger_groups Group_'.strtolower($selTab2[$i]).'_'.$index.' on Group_'.strtolower($selTab2[$i]).'_'.$index.'.groupid=CRM_'
				.strtolower($selTab2[$i]).'_'.$index.'.smownerid ';
				$strf=str_replace(array('CRM_'.strtolower($selTab1[$i]).'_0.smownerid','CRM_'.strtolower($selTab2[$i]).'_'.$index.'.smownerid'), array('Group_'
				.strtolower($selTab1[$i]).'_0.groupname','Group_'.strtolower($selTab2[$i]).'_'.$index.'.groupname'), substr($stringaFields2, 0, -2));
			}
			if ($cftables=='cf') {
				$qjoincf=' JOIN '.$cftable1[$i].' as '.$cftable1[$i].'_0 on '.$cftable1[$i].'_0.'.$firsttblid.'='.strtolower($selTab1[$i]).'_0.'.$firsttblid;
				$q2joincf=' JOIN '.$cftable2[$i].' as '.$cftable2[$i].'_'.$index.' on '.$cftable2[$i].'_'.$index.'.'.$secondtblid.'='
				.strtolower($selTab2[$i]).'_'.$index.'.'.$secondtblid;
				$strf=str_replace(
					array($cftable1[$i].'_'.$index,$cftable2[$i].'_'.$index),
					array($cftable1[$i].'_0',$cftable2[$i].'_'.$index),
					substr($stringaFields2, 0, -2)
				);
			}
			...
			$strQuery .= '<b> FROM </b>' . strtolower($selTab1[$i]) .' as '.strtolower($selTab1[$i]).'_0 join vtiger_crmentity CRM_'
			.strtolower($selTab1[$i]).'_0 on CRM_'.strtolower($selTab1[$i]).'_0.crmid='.strtolower($selTab1[$i]).'_0.'.$firsttblid.
			' <b>'.$qjoin.''.$qjoincf.' </b><b>INNER JOIN </b> '.$selTab2[$i].' <b> as </b> ' . strtolower($selTab2[$i]).'_'.$index. '<b> ON </b>'
			. strtolower($selTab1[$i]).'_0.'. $selfld1 . '<b> = </b>' . strtolower($selTab2[$i]).'_'.$index. '.'. $selfld2.' join vtiger_crmentity CRM_'
			.strtolower($selTab2[$i]).'_'.$index.' on CRM_'.strtolower($selTab2[$i]).'_'.$index.'.crmid='
			.strtolower($selTab2[$i]).'_'.$index.'.'.$secondtblid.' <b>'.$q2join.''.$q2joincf.'</b>';
		 //  if(count($selTab1)==1)
...
//                $strQuery .= '<b> INNER </b> join vtiger_accountbillads as vtiger_accountbillads_'.$index.' <b> ON
// </b> vtiger_account_'.$index.'.accountid=vtiger_accountbillads_'.$index.'.accountaddressid';
//                $strQuery .= ' <b> INNER join </b> vtiger_accountshipads as vtiger_accountshipads_'.$index.' <b> ON
// </b>  vtiger_account_'.$index.'.accountid=vtiger_accountshipads_'.$index.'.accountaddressid';
//                $acc =$acc+ 1;
//            }
//            if (($selTab2[$i] == "vtiger_contactdetails" || $selTab1[$i] == "vtiger_contactdetails" ) && $cont == 0) {
//                $strQuery .= ' <b> INNER JOIN </b> vtiger_contactaddress as vtiger_contactaddress_'.$index.' <b> ON
// </b>  vtiger_contactdetails_'.$index.'.contactid=vtiger_contactaddress_'.$index.'.contactaddressid';
//                $strQuery .= '<b>  INNER JOIN </b> vtiger_contactsubdetails as vtiger_contactsubdetails_'.$index.' <b> ON
// </b>  vtiger_contactdetails_'.$index.'.contactid=vtiger_contactsubdetails_'.$index.'.contactsubscriptionid';
//                $cont =$cont+ 1;
//            }
//            if (($selTab2[$i] == "vtiger_leaddetails" || $selTab1[$i] == "vtiger_leaddetails" ) && $lead == 0) {
//                $strQuery .= ' <b> INNER JOIN </b>vtiger_leadaddress as vtiger_leadaddress_'.$index.' <b> ON
// </b>  vtiger_leaddetails_'.$index.'.leadid=vtiger_leadaddress_'.$index.'.leadaddressid';
//                $strQuery .= '<b>  INNER JOIN </b>vtiger_leadsubdetails as vtiger_leadsubdetails_'.$index.' <b> ON
// </b>  vtiger_leaddetails_'.$index.'.leadid=vtiger_leadsubdetails_'.$index.'.leadsubscriptionid';
//                $lead =$lead+ 1;
//            }
			$index2=$index;
			$index++;
			...
			if ($usergroup=='user') {
				...
				$strf2=str_replace(array('CRM_'.strtolower($selTab1[$i]).'_0.smownerid','CRM_'.strtolower($selTab2[$i]).'_'.$index.'.smownerid'), array('Group_'
				.strtolower($selTab1[$i]).'_0.groupname','Group_'.strtolower($selTab2[$i]).'_'.$index.'.groupname'), substr($stringaFields2, 0, -2));
			}
			if ($cftables=='cf') {
				$qjoin1cf=' JOIN '.$cftable1[$i].' as '.$cftable1[$i]. '_'. $index.' join '.$cftable2[$i]. ' as '.$cftable2[$i].'_'.$index.' on 
				'.$cftable2[$i].'_'.$index.'.'.$secondtblid.'='.$selTab2[$i].'_'.$index.'.'.$secondtblid;
			}
			...
			$strQuery .= '<b> INNER JOIN </b>'.$selTab2[$i].' <b> as </b> ' . $selTab2[$i].'_'.$index . '<b> ON </b>' .
			 strtolower($selTab1[$i]).'_'.($index-1).'.' . $selfld1 . '<b> = </b>' . strtolower($selTab2[$i]).'_'.$index. '.'. $selfld2.' join vtiger_crmentity as CRM_'.
			 $selTab2[$i].'_'.$index.' on CRM_'.$selTab2[$i].'_'.$index.'.crmid='.$selTab2[$i].'_'.$index.'.'.$secondtblid.'<b>'.$qjoin1.' '.$qjoin1cf.'</b>';
			//$strQuery1 .= '<b> INNER JOIN </b>' . $selTab2[$i] . '<b> ON </b>' ;//. strtolower($selTab1[$i]) . '.' . $selField1[$i] . '<b> = </b>' .
			// strtolower($selTab2[$i]) . '.' . $selField2[$i];
//            if ($selTab2[$i] == "vtiger_account" && $acc == 0) {
//                $strQuery .= '<b> INNER </b> join vtiger_accountbillads as vtiger_accountbillads_'.$index.' <b> ON </b> vtiger_account_'.$index.'
// .accountid=vtiger_accountbillads_'.$index.'.accountaddressid';
//                $strQuery .= ' <b> INNER join </b> vtiger_accountshipads as vtiger_accountshipads_'.$index.' <b> ON </b>  vtiger_account_'.$index.
// '.accountid=vtiger_accountshipads_'.$index.'.accountaddressid';
//                $acc =$acc+ 1;
//            }
//            if ($selTab2[$i] == "vtiger_contactdetails" && $cont == 0) {
//                $strQuery .= ' <b> INNER JOIN </b> vtiger_contactaddress as vtiger_contactaddress_'.$index.' <b> ON </b>  vtiger_contactdetails_'.$index.
// '.contactid=vtiger_contactaddress_'.$index.'.contactaddressid';
//                $strQuery .= '<b>  INNER JOIN </b> vtiger_contactsubdetails as vtiger_contactsubdetails_'.$index.' <b> ON </b>  vtiger_contactdetails_'
// .$index.'.contactid=vtiger_contactsubdetails_'.$index.'.contactsubscriptionid';
//                $cont =$cont+ 1;
//            }
//            if ($selTab2[$i] == "vtiger_leaddetails" && $lead == 0) {
//                $strQuery .= ' <b> INNER JOIN </b>vtiger_leadaddress as vtiger_leadaddress_'.$index.' <b> ON </b>  vtiger_leaddetails_'.$index.'
// .leadid=vtiger_leadaddress_'.$index.'.leadaddressid';
//                $strQuery .= '<b>  INNER JOIN </b>vtiger_leadsubdetails as vtiger_leadsubdetails_'.$index.' <b> ON </b>  vtiger_leaddetails_'.$index.
// '.leadid=vtiger_leadsubdetails_'.$index.'.leadsubscriptionid';
//                $lead =$lead+ 1;
//            }
		   //  $strQuery .= inerJoionwithCrmentity($Moduls,$stringaFields,$index,strtolower($selTab1[$i]));
			$index++;
		...
		for ($i = 0; $i <= count($params); $i++) {
			foreach ($Moduls as $modul) {
				...
				if ($splitvalues[1] == "vtiger_crmentity") {
					...
					array_push($Querysplit, $splitvalues[1] . "_0." . $splitvalues[2]);//($nr > 0 ? $splitvalues[1]."_".$nr
					// . "." . $splitvalues[2] : $splitvalues[1] . "." . $splitvalues[2]);
				}
			}
			$index++;
		  //return $Querysplit;
...
//        $prova= selectValues($OptVAl, $modul, $nr);
		...
		if (!empty($JoinCondition)) {
			...
			$joinCrmentity .= ' CRM_' . strtolower($modul) . '.crmid = ' .
			($tab!= $Module_tablename ? $Module_tablename.'_'.$nr.'.'.$Module_entityidfield  : $Module_tablename.'_0.'.$Module_entityidfield);
...
//        if (!(in_array($tableList2[$i], prelevaArray($tableList2, $i)) ||
//  ((in_array($tableList2[$i], prelevaArray($tableList1, $i))) && $tableList2[$i] != $tableList1[0]))) {
