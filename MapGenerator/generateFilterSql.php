...
//$j = 0;
	...
	if (!empty($advft_criteria[$key]['condition'])) {
		$advft_criteria[$key]['condition'] = '';
	}
			...
			foreach ($groupcolumns as $columnindex => $columninfo) {
				...
				if ($fieldcolname != "" && $comparator != "") {//if fieldscolname different from null
					...
					$concatSql = getSqlForNameInDisplayFormat(
						array('first_name'=>$selectedfields[0].".first_name",'last_name'=>$selectedfields[0].".last_name"),
						'Users',
						$dbname
					);
...
//                        }
					...
					if (isset($valuearray) && count($valuearray) > 1 && $comparator != 'bw') {
						...
						for ($n=0; $n<count($valuearray); $n++) {
							...
							$selectedfields[0] == "vtiger_users".$this->secondarymodule) && $selectedfields[1] == 'user_name') {
								...
								or vtiger_groups".$module_from_tablename.".groupname ".$this->getAdvComparator($comparator, trim($valuearray[$n]), $datatype);
					...
					} //elseif(($selectedfields[0] == "vtiger_users".$this->primarymodule ||
// $selectedfields[0] == "vtiger_users".$this->secondarymodule) && $selectedfields[1] == 'user_name') {
//							$module_from_tablename = str_replace("vtiger_users","",$selectedfields[0]);
//							$fieldvalue = " trim(case when (".$selectedfields[0].".last_name NOT LIKE '')
//  then ".$concatSql." else vtiger_groups".$module_from_tablename.".groupname end) "
					...
					elseif ($selectedfields[2] == 'Quotes_Inventory_Manager') {
						...
						if ($module_from_tablename != '') {
									$tableName = 'vtiger_lastModifiedBy'.$module_from_tablename;
																						...
																						$fieldvalue = getSqlForNameInDisplayFormat(array('last_name'=>
																						"$tableName.last_name",'first_name'=>"$tableName.first_name"), 'Users').
...
//                                                elseif($selectedfields[0] == 'vtiger_inventoryproductrel' && ($selectedfields[1] ==
//  'productid' || $selectedfields[1] == 'serviceid')) {
...
//						}
	...
	if (trim($advfiltersql) != "") {
		$advfiltersql = ''.$advfiltersql.'';//second brackets
	}
		return $advfiltersql;
}
