...
function GetModulRel($m) {
	...
	$j = 0;
	$result = $adb->pquery(
		"SELECT relmodule,columnname,fieldlabel 
	...
	if ($num_rows != 0) {
		for ($i = 1; $i <= $num_rows; $i++) {
			...
			if (strlen($FirstmoduleXML) != 0 && $FirstmoduleXML == $modul1) {
				$a .= '<option selected value="' . $modul1 . ';' . $column . '">' .
				str_replace("'", "", getTranslatedString($modul1)) . ' ' .
				str_replace("'", "", getTranslatedString($fl, $modul1)) . '</option>';
			} else {
				$a .= '<option value="' . $modul1 . ';' . $column . '">' .
				str_replace("'", "", getTranslatedString($modul1)) . ' ' .
				str_replace("'", "", getTranslatedString($fl, $modul1)) . '</option>';
			}
			// echo $modul1;
	...
	if ($num_rows1 != 0) {
		for ($i = 1; $i <= $num_rows1; $i++) {
			...
			if (strlen($FirstmoduleXML) != 0 && $FirstmoduleXML == $modul1) {
				$a .= '<option selected value="' . $modul1 . '(many);' . $column . '">' .
				str_replace("'", "", getTranslatedString($modul1)) . '(' . $mod_strings['many'] . ')' .
				str_replace("'", "", getTranslatedString($fl, $modul1)) . '</option>';
			} else {
				$a .= '<option value="' . $modul1 . '(many);' . $column . '">' .
				str_replace("'", "", getTranslatedString($modul1)) . '(' . $mod_strings['many'] . ')' .
				str_replace("'", "", getTranslatedString($fl, $modul1)) . '</option>';
			}
		}
		...
		for ($i = 1; $i <= $num_rows2; $i++) {
			...
			if ($modul2 != '' && $adb->num_rows($mo2) != 0) {
				if (strlen($FirstmoduleXML) != 0 && $FirstmoduleXML == $modul2) {
					$a .= '<option selected value="' . $modul2 . '; ' . $column . '">' .
					str_replace("'", "", getTranslatedString($modul2)) . ' ' .
					str_replace("'", "", getTranslatedString($fl, $modul2)) . '</option>';
				} else {
					$a .= '<option value="' . $modul2 . '; ' . $column . '">' .
					str_replace("'", "", getTranslatedString($modul2)) . ' ' .
					str_replace("'", "", getTranslatedString($fl, $modul2)) . '</option>';
				}
			}
	...
	if ($m == "SalesOrder") {
		$query2 = "SELECT columnname, name, fieldlabel from  vtiger_field join  vtiger_tab on 
		vtiger_tab.tabid= vtiger_field.tabid where uitype=81 and uitype=75 and name<>'$m'
        and name<>'Faq' and name<>'Emails' and name<>'Events' and name<>'Webmails' and name<>'SMSNotifier'
        and name<>'PBXManager' and name<>'Modcomments' and name<>'Calendar' and  vtiger_tab.presence=0
    ...
    ";
		...
		if ($num_rows2 != 0) {
			for ($i = 1; $i <= $num_rows2; $i++) {
				...
				if ($adb->num_rows($mo) != 0) {
					if (strlen($FirstmoduleXML) != 0 && $FirstmoduleXML == $modul1) {
						$a .= '<option selected="selected" value="' . $modul1 . '(many); ' . $column . '">' .
						str_replace("'", "", getTranslatedString($modul1)) . '(' . $mod_strings['many'] . '); ' .
						str_replace("'", "", getTranslatedString($fl, $modul1)) . '</option>';
					} else {
						$a .= '<option  value="' . $modul1 . '(many); ' . $column . '">' .
...
function GetModulRelOneTomulti($m, $valuefromLoad = "") {
	...
	if ($m == "Campaigns") {
		$query2 = "SELECT columnname, name, fieldlabel from  vtiger_field join  vtiger_tab on  vtiger_tab.tabid= vtiger_field.tabid where 
		uitype=58 and name<>'$m'
        and name<>'Faq' and name<>'Emails' and name<>'Events' and name<>'Webmails' and name<>'SMSNotifier'
        and name<>'PBXManager' and name<>'Modcomments' and name<>'Calendar' and  vtiger_tab.presence=0
    ";
	...
	if ($m == "SalesOrder") {
		$query2 = "SELECT columnname, name, fieldlabel from  vtiger_field join  vtiger_tab on  vtiger_tab.tabid= vtiger_field.tabid where 
		uitype=81 and uitype=75 and name<>'$m'
        and name<>'Faq' and name<>'Emails' and name<>'Events' and name<>'Webmails' and name<>'SMSNotifier'
        and name<>'PBXManager' and name<>'Modcomments' and name<>'Calendar' and  vtiger_tab.presence=0
...
function GetModulToAll($m, $valuefromLoad = "") {
	...
	if ($m == "SalesOrder") {
		$query2 = "SELECT columnname, name, fieldlabel from  vtiger_field join  vtiger_tab on  vtiger_tab.tabid= vtiger_field.tabid where uitype=81 and 
		uitype=75 and name<>'$m'
        and name<>'Faq' and name<>'Emails' and name<>'Events' and name<>'Webmails' and name<>'SMSNotifier'
        and name<>'PBXManager' and name<>'Modcomments' and name<>'Calendar' and  vtiger_tab.presence=0
	...
	if ($num_rows != 0) {
		for ($i = 1; $i <= $num_rows; $i++) {
			...
			if (strlen($valuefromLoad) != 0 && $valuefromLoad == $modul1) {
				$a= $modul1 . ';' . $column . '#' .
				str_replace("'", "", getTranslatedString($modul1)) . ' ' .
				str_replace("'", "", getTranslatedString($fl, $modul1));
			}
			// echo $modul1;
		}
	}
...
function Check_table_if_exist($tableName, $primaryIds = "") {
		global $adb;
		$showTable = $adb->query("SHOW TABLES LIKE '$tableName'");
		$exist=$adb->query_result($showTable, 0, 0);
	if (strlen($exist)==0) {
		$createTable="
...
function showError($TitleError = '', $BoddyError = '') {
	...
	include_once 'include/CustomFieldUtil.php';
...
function GetAllRelationDuplicaterecords($module = "") {
	...
	if (!empty($module)) {
		...
		if ($num_rows!=0) {
			for ($i=1; $i<=$num_rows; $i++) {
				$modulename=(!empty(SearchbyIDModule($adb->query_result($result, $i-1, 'related_tabid')))
				?SearchbyIDModule($adb->query_result($result, $i-1, 'related_tabid')):
				$adb->query_result($result, $i-1, 'label'));
				$Modules = $adb->query_result($result, $i-1, 'label');
				$relatedtypes = $adb->query_result($result, $i-1, 'relationtype');

				$a.='<option value="'.$modulename.'#'.$relatedtypes.'">'.
				str_replace("'", "", getTranslatedString($modulename)).'</option>';
			}
			return $a;
		} else {
...
/**
 * function for mapping(!@shibe2018)
 *
 * @param <type> $module The module
 */
