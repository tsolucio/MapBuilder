...
if (empty($GetALLMaps)) {
	echo showError(getTranslatedString('wrong_action'), getTranslatedString('map_type_missing'));
}
...
if ($MypType=="Mapping") {
	try {
		...
		$log->debug(TypeOFErrors::ERRORLG."Something was wrong check the Exception ".$ex);
		 echo showError(getTranslatedString('error'), getTranslatedString('error_check_exceptionLog'));
		 LogFile($ex);
	...
	try {
		...
		$log->debug(TypeOFErrors::ERRORLG."Something was wrong check the Exception ".$ex);
		echo showError(getTranslatedString('wrong_action'), $ex->getMessage());
		LogFile($ex);
		// echo TypeOFErrors::ERRORLG."Something was wrong check the Exception ".$ex;
	}
} elseif ($MypType==="ListColumns") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="Condition Query") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="Module Set Mapping") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="FieldDependency") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="FieldDependencyPortal") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="GlobalSearchAutocomplete") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="Condition Expression") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="CREATEVIEWPORTAL") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="DETAILVIEWBLOCKPORTAL") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="Record Access Control") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="DuplicateRecords") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="RendicontaConfig") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="Record Set Mapping") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
...
} elseif ($MypType==="Extended Field Information") {
	try {
		if (!empty($QueryHistory) || !empty($MapID)) {
	...
	echo showError(getTranslatedString('wrong_action'), getTranslatedString('not_exist_type_map_load').'\n'.getTranslatedString('check_map_type_try_again'));
}
...
function FieldSet($QueryHistory, $MapID) {
	...
	try {
		if (!empty($QueryHistory)) {
			...
			global $adb;
			$sql = 'SELECT mapname, content, maptype FROM vtiger_cbmap WHERE cbmapid=?';
			$result = $adb->pquery($sql, array($MapID));
			$mapName=$adb->query_result($result, 0, 'mapname');
			$contents=$adb->query_result($result, 0, 'content');
			$mapType=$adb->query_result($result, 0, 'maptype');
			$smarty = new vtigerCRM_Smarty();
			$smarty->assign("mapName", $mapName);
			...
			$smarty->assign("MOD", $mod_strings);
			$smarty->assign("APP", $app_strings);
			$output = $smarty->fetch('modules/MapGenerator/map.tpl');
			echo $output;
		}
	} catch (Exception $ex) {
		echo showError(getTranslatedString('wrong_action'), $ex->getMessage());
...
function RecordSetMapping($QueryHistory, $MapID) {
	...
	try {
		if (!empty($QueryHistory)) {
			...
			$smarty->assign("MOD", $mod_strings);
			$smarty->assign("APP", $app_strings);
			$output = $smarty->fetch('modules/MapGenerator/TemplateDesignMapGEnerator.tpl');
			echo $output;
		}
	} catch (Exception $ex) {
		echo showError(getTranslatedString('wrong_action'), $ex->getMessage());
...
function MENUSTRUCTURE($QueryHistory, $MapID) {
	...
	if (!empty($QueryHistory)) {
		...
		$smarty->assign("MOD", $mod_strings);
		$smarty->assign("APP", $app_strings);
		$output = $smarty->fetch('modules/MapGenerator/LoadTemplateMap/map.tpl');
		echo $output;
	}
}
...
function Mapping_View($QueryHistory, $MapID) {
	...
	try {
		if (!empty($QueryHistory)) {
			...
			foreach ($Allhistory as $value) {
				...
				foreach ($xml->fields->field as $field) {
					$araymy=[
						...
						'FirstModuleval' =>explode("#", Get_First_Moduls_TextVal($xml->originmodule[0]->originmodule))[0],

						'FirstModuletxt' =>explode("#", Get_First_Moduls_TextVal($xml->originmodule[0]->originmodule))[1],

						'SecondModuletxt' =>explode("#", Get_First_Moduls_TextVal($xml->targetmodule[0]->targetmodule))[1],

						'SecondModuleval' =>explode("#", Get_First_Moduls_TextVal($xml->targetmodule[0]->targetmodule))[1],
					...
					} elseif (!empty(CheckAllFirstForAllModules($field->Orgfields->Orgfield->OrgfieldName))) {
			...
			$smarty->assign("FirstModuleSelected", $FirstModuleSelected);
			$smarty->assign("SecondModuleSelected", $SecondModuleSelected);
		...
		showError(getTranslatedString('wrong_action'), getTranslatedString('missing_mapID_queryID'));
	}
}
...
function GetAllrelation1TOManyMaps($module = "", $selectedmodule = "") {
	...
	if (!empty($module)) {
		...
		if ($num_rows!=0) {
			...
			$log->debug("Info!! The database is empty or something was wrong");
		}
	} else {
		return "";
	}
