...
function add_content($DataDecode) {
	...
	$targetnameText = $xml->createTextNode(trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace("(many)", '', preg_replace(
		'/\s+/',
		'',
		explode(";", $DataDecode[0]->temparray->secmodule)[0]
	)))));
	...
	for ($i = 0; $i <= $countarray; $i++) {
		...
		} elseif ($DataDecode[$i]->temparray->JsonType == "Popup Screen") {
...
function add_aray_for_history($decodedata) {
	...
	return array
		...
		'SecondModuleval' => trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace("(many)", '', preg_replace(
			'/\s+/',
			'',
			explode(";", $decodedata[0]->temparray->secmodule)[0]
		)))),
...
function save_history($datas, $queryid, $xmldata) {
	...
	if (!empty($seq)) {
		...
		//$seqmap=count($data);
		$adb->pquery(
			"insert into " . TypeOFErrors::TABELE_NAME . " values (?,?,?,?,?,?,?,?,?,?,?)",
			array($idquery, $datas["FirstModuleval"], $datas["FirstModuletxt"], $datas["SecondModuletxt"],
			$datas["SecondModuleval"],
			$xmldata,
			$seq,
			1,
			$datas["firstmodulelabel"],
			$datas["secondmodulelabel"],
			$datas["Labels"])
		);
		//return $idquery;
	} else {
		...
		$adb->pquery(
			"insert into " . TypeOFErrors::TABELE_NAME . " values (?,?,?,?,?,?,?,?,?,?,?)",
			array($idquery, $datas["FirstModuleval"], $datas["FirstModuletxt"], $datas["SecondModuletxt"], $datas["SecondModuleval"],
			$xmldata,
			1,
			1,
			$datas["firstmodulelabel"],
