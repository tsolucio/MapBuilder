...
$SaveasMapText = $_POST['SaveasMapText']; // stringa con tutti i campi scelti in selField1 (string with all the fields chosen in selField1)
	...
	if (strlen($MapID[1]) == 0) {
		...
		if (!$focust->saveentity("cbMap")) { //
			if (Check_table_if_exist(TypeOFErrors::TABELE_NAME) > 0) {
				echo save_history(add_aray_for_history($decodedata), $focust->column_fields['mvqueryid'], $focust->column_fields['description']) . "," . $focust->id;
			} else {
				echo "0,0";
		...
		if (Check_table_if_exist(TypeOFErrors::TABELE_NAME) > 0) {
			echo save_history(add_aray_for_history($decodedata), $focust->column_fields['mvqueryid'], $focust->column_fields['description']) . "," . $MapID[1];
		} else {
			echo "0,0";
			$log->debug("Error!! MIssing the history Table");
		}
...
function add_content($DataDecode) {
	...
	$targetnameText = $xml->createTextNode(trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace(
		"(many)",
		'',
		preg_replace('/\s+/', '', explode(";", $DataDecode[0]['SecondModuleval'])[0])
	))));
...
function add_description($DataDecode) {
	...
	$SecmodulenameText = $xml->createTextNode(trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace("(many)", '', preg_replace(
		'/\s+/',
		'',
		explode(";", $DataDecode[0]['SecondModuleval'])[0]
	)))));
...
function save_history($datas, $queryid, $xmldata) {
	...
	if (!empty($seq)) {
		...
		$adb->pquery("insert into " . TypeOFErrors::TABELE_NAME . " values (?,?,?,?,?,?,?,?,?,?,?)", array($idquery2, $datas["FirstModuleval"], $datas["FirstModuletxt"],
		 $datas["SecondModuletxt"], $datas["SecondModuleval"], $xmldata, $seq, 1, "", "", $datas["Labels"]));
		//return $idquery;
	} else {
		$adb->pquery("insert into " . TypeOFErrors::TABELE_NAME . " values (?,?,?,?,?,?,?,?,?,?,?)", array($idquery2, $datas["FirstModuleval"], $datas["FirstModuletxt"],
		 $datas["SecondModuletxt"], $datas["SecondModuleval"], $xmldata, 1, 1, "", "", $datas["Labels"]));
	}
	echo $idquery2;
}
...
function add_aray_for_history($decodedata) {
	...
	for ($i = 0; $i <= $countarray; $i++) {
		$labels;
		$labels .= $decodedata[0]['FirstModuletxt'] . ":" . explode(":", $decodedata[$i]['FirstFieldval'])[0] . ":" . explode(":", $decodedata[$i]['SecondFieldval'])[2]
		 . "," . trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace("(many)", '', preg_replace('/\s+/', '', explode(";", $decodedata[0]['SecondModuleval'])[0])))) .
		  ":" . explode(":", $decodedata[$i]['SecondFieldval'])[0] . ":" . explode(":", $decodedata[$i]['SecondFieldval'])[2] . ",";
	}
	return array
		(
		'Labels' => $labels,
		...
		'SecondModuletxt' => trim(preg_replace('/\s*\([^)]*\)/', '', preg_replace("(many)", '', preg_replace(
			'/\s+/',
			'',
			explode(";", $decodedata[0]['SecondModuleval'])[0]
		)))),
	);
}
...
function findIdrelationAndName($NameofModul, $RelModul) {
	...
	if (!empty($NameofModul) && !empty($RelModul)) {
		...
		WHERE module='$NameofModul' and relmodule='$RelModul'");
